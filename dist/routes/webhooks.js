...
const whatsappAdapters_1 = require("../adapters/whatsappAdapters");
...
router.post("/whatsapp", async (req, res) => {
    ...
    try {
        const message = (0, whatsappAdapters_1.normalizePayload)(body, provider);
        if (!message) {
            ...
            logger_1.log.webhook("descartado", { motivo });
            res.status(200).json({ received: true, processed: false, reason: motivo });
            return;
        }
        logger_1.log.webhook("normalizado", {
            telefone: message.telefone,
            texto: message.texto,
            messageId: message.messageId,
        });
        const result = await (0, whatsappService_1.processWhatsAppMessage)(message);
        const elapsed = Date.now() - start;
        if (result.success) {
            logger_1.log.webhook("concluido", {
                elapsed: `${elapsed}ms`,
            ...
            logger_1.log.webhook("falhou", { elapsed: `${elapsed}ms`, motivo: result.erro });
        }
        res.status(200).json({
            received: true,
        ...
        const elapsed = Date.now() - start;
        logger_1.log.error("erro interno no webhook", err, { provider, elapsed: `${elapsed}ms` });
...
async function fetchAsaasCustomerPhone(customerId) {
...
router.post("/asaas", async (req, res) => {
    try {
        ...
        if (paymentId) {
           ...
           VALUES ($1, $2)
           ON CONFLICT (message_id) DO NOTHING`, [`asaas_payment_${paymentId}`, "webhook_asaas"]);
                if (dedup.rowCount === 0) {
        ...
        let telefoneRaw = payment?.externalReference;
        ...
        const variants = new Set([telefone]);
        const expand = (t) => {
            if (t.length === 13) { // 55+DDD+9+local
                const s = t.slice(2);
                variants.add(s); // DDD+9+local
                const n = s.slice(0, 2) + s.slice(3);
                variants.add(n);
                variants.add("55" + n); // sem 9
            }
            else if (t.length === 12) { // 55+DDD+local
                const s = t.slice(2);
                variants.add(s); // DDD+local
                const c = s.slice(0, 2) + "9" + s.slice(2);
                variants.add(c);
                variants.add("55" + c); // com 9
            }
            else if (t.length === 11) { // DDD+9+local
                variants.add("55" + t);
                const n = t.slice(0, 2) + t.slice(3);
                variants.add(n);
                variants.add("55" + n);
            }
            else if (t.length === 10) { // DDD+local
                variants.add("55" + t);
                const c = t.slice(0, 2) + "9" + t.slice(2);
                variants.add(c);
                variants.add("55" + c);
            }
        };
        ...
        const description = (payment?.description ?? payment?.billingType ?? "").toLowerCase();
        const planRow = await client_1.default.query(`SELECT nome, duration_days FROM plans WHERE ativo = true AND $1 ILIKE '%' || nome || '%' LIMIT 1`, [description]);
        ...
        const result = await client_1.default.query(`WITH prev AS (
       ...
       FROM prev
       WHERE u.id = prev.id
       RETURNING u.id, u.telefone, prev.prev_status`, [variantsArr, plan.duration_days.toString(), plan.nome]);
        ...
        logger_1.log.webhook("asaas: usuario ativado", { userId: user.id, telefone: user.telefone, paymentId: payment?.id, isRenovacao });
        ...
        try {
            await whatsapp_1.whatsapp.sendText({ to: user.telefone, text: texto });
        }
        catch (err) {
