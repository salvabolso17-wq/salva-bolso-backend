...
function parseValor(raw) {
    ...
    if (milNum)
        return parseValorBase(milNum[1]) * 1000;
...
function parseTransaction(texto) {
    ...
    const valorMatch = textoParse.match(/R?\$?\s*(\d{1,3}(?:\.\d{3})+(?:,\d{1,2})?|\d+(?:[.,]\d{1,2})?\s*mil|\d+(?:[.,]\d{1,2})?)/i);
    if (!valorMatch)
        return null;
    const valor = parseValor(valorMatch[1]);
    if (isNaN(valor) || valor <= 0)
        return null;
    const descricao = normalizeDescricao(cleanDescricao(textoParse.replace(valorMatch[0], "").replace(/\+/g, "").trim() || "Sem descrição"));
    ...
    for (const [categoria, config] of Object.entries(CATEGORIAS)) {
        for (const keyword of config.keywords) {
            ...
            if (kw.length <= bestLen)
                continue; // só processa se puder melhorar
