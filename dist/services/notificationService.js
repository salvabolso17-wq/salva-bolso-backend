...
exports.runWeeklyNotifications = runWeeklyNotifications;
exports.runDailyNotifications = runDailyNotifications;
const client_1 = __importDefault(require("../db/client"));
const whatsapp_1 = require("./whatsapp");
const logger_1 = require("../utils/logger");
const reportService_1 = require("./reportService");
const plansMessage_1 = require("../utils/plansMessage");
...
async function tryMarkSent(userId, categoria, mesRef) {
    const result = await client_1.default.query(`INSERT INTO sent_insights (user_id, categoria, marco, mes_referencia)
     VALUES ($1, $2, 1, $3)
     ON CONFLICT (user_id, categoria, marco, mes_referencia) DO NOTHING`, [userId, categoria, mesRef]);
...
async function sendInactivityNotifications() {
  ...
  `, [sentinel]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "notif_inatividade", sentinel)))
            continue;
        try {
            ...
            logger_1.log.whatsapp("notif inatividade enviada", { to: user.telefone, userId: user.id });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar notif inatividade", err, { userId: user.id });
        }
    }
}
// Últimos dias do mês para usuários com ≥3 gastos no mês → lembra de ver resumo (máx 1x/mês)
async function sendMonthEndNotifications() {
    if (new Date().getUTCDate() < 26)
        return;
  ...
  `, [sentinel]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "notif_fim_mes", sentinel)))
            continue;
        try {
            await whatsapp_1.whatsapp.sendText({
                to: user.telefone,
            ...
            logger_1.log.whatsapp("notif fim de mes enviada", { to: user.telefone, userId: user.id });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar notif fim de mes", err, { userId: user.id });
        }
    }
}
// Meta com < 80% de progresso criada há > 7 dias → incentiva depósito (máx 1x/semana, 1 meta/usuário)
async function sendGoalStagnationNotifications() {
    const sentinel = currentWeekStart();
    const { rows } = await client_1.default.query(`
  ...
  `, [sentinel]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "notif_meta_parada", sentinel)))
            continue;
        try {
            await whatsapp_1.whatsapp.sendText({
                to: user.telefone,
            ...
            logger_1.log.whatsapp("notif meta parada enviada", { to: user.telefone, userId: user.id, meta: user.nome });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar notif meta parada", err, { userId: user.id });
        }
    }
}
// Dia 1 do mês: resume o mês anterior com sensação de progresso (máx 1x/mês por usuário)
async function sendMonthlyClosingNotifications() {
  ...
  `, [prevStart, prevEnd]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "notif_fechamento", sentinel)))
            continue;
        try {
            ...
            await whatsapp_1.whatsapp.sendText({ to: user.telefone, text: linhas.join("\n") });
            logger_1.log.whatsapp("notif fechamento enviada", { to: user.telefone, userId: user.id, mes: mesNome });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar notif fechamento", err, { userId: user.id });
        }
    }
}
// Dia 1 do mês: inicia fluxo interativo para renda + carryover de saldo (máx 1x/mês por usuário)
async function sendNewMonthFlowNotifications() {
    if (new Date().getUTCDate() !== 1)
        return;
    const now = new Date();
    const prevStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 1, 1));
  ...
  `, [prevStart, prevEnd]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "notif_novo_mes", sentinel)))
            continue;
        const inserted = await client_1.default.query(`INSERT INTO pending_actions (user_id, action, step, tx_ids, expires_at)
       ...
       ON CONFLICT (user_id) DO NOTHING`, [user.id]);
        ...
        try {
            ...
            logger_1.log.whatsapp("notif novo mes enviada", { to: user.telefone, userId: user.id, mes: mesAtual });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar notif novo mes", err, { userId: user.id });
        }
    }
}
...
function buildReminderText(daysAhead, plansBlock) {
...
async function sendSubscriptionReminders() {
    const plansBlock = await (0, plansMessage_1.buildPlansBlock)();
    for (const { daysAhead, marco } of SUBSCRIPTION_REMINDERS) {
        let rows;
        try {
            ({ rows } = await client_1.default.query(`SELECT u.id, u.telefone,
         ...
         WHERE u.subscription_status = 'active'
           ...
           AND NOT EXISTS (
            ...
            logger_1.log.error(`falha ao buscar usuarios para sub_reminder ${daysAhead}d`, err);
            continue;
        }
        for (const user of rows) {
            try {
           ...
           ON CONFLICT (user_id, categoria, marco, mes_referencia) DO NOTHING`, [user.id, marco, user.expires_date]);
                if ((inserted.rowCount ?? 0) === 0)
                    continue;
                await whatsapp_1.whatsapp.sendText({ to: user.telefone, text: buildReminderText(daysAhead, plansBlock) });
                logger_1.log.whatsapp(`sub_reminder ${daysAhead}d enviado`, { to: user.telefone, userId: user.id });
            }
            catch (err) {
                logger_1.log.error(`falha sub_reminder ${daysAhead}d`, err, { userId: user.id });
            }
        }
    }
}
...
function buildTrialReminderText(daysAhead, plansBlock) {
        ...
        try {
         ...
         WHERE u.subscription_status = 'trial'
           ...
           AND NOT EXISTS (
            ...
            logger_1.log.error(`falha ao buscar usuarios para trial_reminder ${daysAhead}d`, err);
            continue;
        }
        for (const user of rows) {
            try {
           ...
           ON CONFLICT (user_id, categoria, marco, mes_referencia) DO NOTHING`, [user.id, marco, user.trial_date]);
                if ((inserted.rowCount ?? 0) === 0)
                    continue;
                await whatsapp_1.whatsapp.sendText({ to: user.telefone, text: buildTrialReminderText(daysAhead, plansBlock) });
                logger_1.log.whatsapp(`trial_reminder ${daysAhead}d enviado`, { to: user.telefone, userId: user.id });
            }
            catch (err) {
                logger_1.log.error(`falha trial_reminder ${daysAhead}d`, err, { userId: user.id });
            }
        }
    }
}
// Dia 25: alerta proativo de recorrentes do próximo mês (diferente de "próximas" que é sob demanda)
async function sendRecorrentesAlert() {
  ...
  `, [sentinel]);
    for (const user of rows) {
        if (!(await tryMarkSent(user.id, "alerta_recorrentes", sentinel)))
            continue;
        try {
            const recRows = await client_1.default.query(`SELECT nome, valor FROM recurring_expenses WHERE user_id = $1 AND ativo = TRUE ORDER BY valor DESC`, [user.id]);
            ...
            logger_1.log.whatsapp("alerta recorrentes enviado", { to: user.telefone, userId: user.id, total });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar alerta recorrentes", err, { userId: user.id });
        }
    }
}
// Dia 1: score financeiro mensal — enviado junto ao fechamento
async function sendMonthlyScore() {
    if (new Date().getUTCDate() !== 1)
        return;
    const now = new Date();
    const prevStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 1, 1));
  ...
  `, [prevStart, prevEnd]);
    for (const user of rows) {
        try {
         ...
         ON CONFLICT (user_id, categoria, marco, mes_referencia) DO NOTHING`, [user.id, prevStart]);
            if ((inserted.rowCount ?? 0) === 0)
                continue;
            // Coleta dados para o score
            const [totaisRes, limRes, metasRes, recRes, txRes] = await Promise.all([
                client_1.default.query(`SELECT COALESCE(SUM(CASE WHEN tipo='entrada' THEN valor ELSE 0 END),0) AS entradas,
                  COALESCE(SUM(CASE WHEN tipo='saida'   THEN valor ELSE 0 END),0) AS saidas
           FROM transactions WHERE user_id = $1 AND criado_em >= $2 AND criado_em < $3`, [user.id, prevStart, prevEnd]),
                client_1.default.query(`SELECT COUNT(*) AS excedidos FROM category_limits cl
           WHERE cl.user_id = $1
             AND (SELECT COALESCE(SUM(valor),0) FROM transactions
                  WHERE user_id = $1 AND LOWER(categoria) = LOWER(cl.categoria)
                    AND tipo='saida' AND criado_em >= $2 AND criado_em < $3) > cl.valor_limite`, [user.id, prevStart, prevEnd]),
                client_1.default.query(`SELECT COUNT(*) AS count FROM user_goals WHERE user_id = $1 AND valor_atual > 0`, [user.id]),
                client_1.default.query(`SELECT COUNT(*) AS count FROM recurring_expenses WHERE user_id = $1 AND ativo = TRUE`, [user.id]),
                client_1.default.query(`SELECT COUNT(*) AS count FROM transactions
           WHERE user_id = $1 AND tipo='saida' AND criado_em >= $2 AND criado_em < $3`, [user.id, prevStart, prevEnd]),
            ]);
            ...
            if (rendaTotal > 0 && saidas <= rendaTotal) {
                score += 3;
                criterios.push("✅ Ficou dentro do orçamento");
            }
            else {
                criterios.push("⚠️ Gastos acima da renda");
            }
            if (excedidos === 0 && (await client_1.default.query(`SELECT COUNT(*) AS c FROM category_limits WHERE user_id = $1`, [user.id])).rows[0].c > 0) {
                score += 2;
                criterios.push("✅ Nenhum limite excedido");
            }
            else if (excedidos > 0) {
            ...
            if (temMetas) {
                score += 2;
                criterios.push("✅ Metas ativas");
            }
            if (txCount >= 10) {
                score += 2;
                criterios.push("✅ Controle consistente");
            }
            else if (txCount >= 5) {
                score += 1;
                criterios.push("✅ Bom registro de gastos");
            }
            if (temRec) {
                score += 1;
                criterios.push("✅ Recorrentes organizados");
            }
            ...
            logger_1.log.whatsapp("score mensal enviado", { to: user.telefone, userId: user.id, score });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar score mensal", err, { userId: user.id });
        }
    }
}
...
async function runWeeklyNotifications() {
    ...
    if (dow !== 1)
        return; // garante que só roda na segunda (proteção extra)
  ...
  `, [semIni, semFim, sentinel]);
    for (const user of rows) {
        try {
         ...
         ON CONFLICT (user_id, categoria, marco, mes_referencia) DO NOTHING`, [user.id, sentinel]);
            if ((inserted.rowCount ?? 0) === 0)
                continue;
            const [cur, prev] = await Promise.all([
                client_1.default.query(`SELECT COALESCE(SUM(valor),0)::text AS total,
           ...
           FROM transactions WHERE user_id = $1 AND tipo='saida' AND criado_em >= $2 AND criado_em < $3`, [user.id, semIni, semFim]),
                client_1.default.query(`SELECT COALESCE(SUM(valor),0)::text AS total FROM transactions
           WHERE user_id = $1 AND tipo='saida' AND criado_em >= $2 AND criado_em < $3`, [user.id, antIni, semIni]),
            ]);
            ...
            if (comparativo)
                linhas.push(comparativo);
            if (topCat)
                linhas.push("", `${emoji} Maior gasto: ${topCat}`);
            linhas.push("", 'Envie "resumo" para ver por categoria.');
            await whatsapp_1.whatsapp.sendText({ to: user.telefone, text: linhas.join("\n") });
            logger_1.log.whatsapp("push semanal enviado", { to: user.telefone, userId: user.id, totalSem });
        }
        catch (err) {
            logger_1.log.error("falha ao enviar push semanal", err, { userId: user.id });
        }
    }
}
...
async function runDailyNotifications() {
    ...
    try {
        ...
        logger_1.log.error("falha sub reminders", err);
    }
    try {
        await sendTrialReminders();
    }
    catch (err) {
        logger_1.log.error("falha trial reminders", err);
    }
    try {
        await sendRecorrentesAlert();
    }
    catch (err) {
        logger_1.log.error("falha alerta recorrentes", err);
    }
    try {
        await sendInactivityNotifications();
    }
    catch (err) {
        logger_1.log.error("falha notif inatividade", err);
    }
    try {
        await sendMonthEndNotifications();
    }
    catch (err) {
        logger_1.log.error("falha notif fim mes", err);
    }
    try {
        await sendGoalStagnationNotifications();
    }
    catch (err) {
        logger_1.log.error("falha notif meta parada", err);
    }
    try {
        await sendMonthlyClosingNotifications();
    }
    catch (err) {
        logger_1.log.error("falha notif fechamento", err);
    }
    try {
        await sendMonthlyScore();
    }
    catch (err) {
        logger_1.log.error("falha score mensal", err);
    }
    try {
        await sendNewMonthFlowNotifications();
    }
    catch (err) {
        logger_1.log.error("falha notif novo mes", err);
    }
    logger_1.log.webhook("notificações diárias concluídas");
}
