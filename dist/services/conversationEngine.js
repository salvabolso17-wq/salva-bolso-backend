...
function hasRecentAction(userId, action) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return false;
...
function canSendInsight(userId, cooldownMs = 10 * 60 * 1000) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return true;
    if (!session.lastInsightAt)
        return true;
    return Date.now() - session.lastInsightAt.getTime() > cooldownMs;
}
function recordInsightSent(userId) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return;
...
function getContextualNextStep(session, txCountDb) {
...
function setLastCommand(userId, command) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return;
...
function setLastInstallment(userId, info) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return;
...
function getLastInstallment(userId) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return null;
    return session.lastInstallment;
}
function setLastGoal(userId, info) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return;
...
function setLastContext(userId, ctx) {
    const session = SESSIONS.get(userId);
    if (!session || isExpired(session))
        return;
