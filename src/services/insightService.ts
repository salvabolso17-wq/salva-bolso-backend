...
export interface Insight {
  tipo: "positivo" | "alerta" | "critico" | "informativo";
  titulo: string;
  descricao: string;
  categoria?: string;
  valor?: number;
...
export interface IndicadoresFinanceiros {
  taxa_economia_percentual: number;
  media_gastos_diarios: number;
  dia_semana_maior_gasto: string | null;
  categoria_mais_frequente: string | null;
  ticket_medio_saida: number;
...
export interface InsightsData {
  ...
  insights_textuais: Insight[];
  alertas: Insight[];
  indicadores_financeiros: IndicadoresFinanceiros;
...
function classifyScore(score: number): ScoreFinanceiro["classificacao"] {
...
function diasNoPeriodo(inicio: Date, fim: Date): number {
  return Math.max(1, Math.round((fim.getTime() - inicio.getTime()) / 86_400_000));
}
...
interface RawPeriodData {
  ...
  qtd_saidas: number;
  categorias: Array<{ categoria: string; total: number; qtd: number }>;
  dia_semana_top: number | null;
  goals: Array<{ categoria: string; valor_meta: number; valor_gasto: number }>;
}
...
async function fetchRawData(
  ...
  const [totaisRes, catRes, diaRes, goalsRes] = await Promise.all([
    ...
    pool.query(
      `SELECT EXTRACT(DOW FROM criado_em)::INT AS dia_num, SUM(valor) AS total
       FROM transactions
       WHERE user_id = $1 AND tipo = 'saida' AND criado_em >= $2 AND criado_em < $3
       GROUP BY dia_num
       ORDER BY total DESC
    ...
    pool.query(
      `SELECT
       ...
       GROUP BY fg.id, fg.categoria, fg.valor_meta`,
      [userId, mesRef]
    ),
  ]);
...
function calcScore(
  ...
  const taxaEcon = curr.total_entradas > 0
    ...
    : 0;
  let ptsEcon = 0;
  if (taxaEcon >= 30)      ptsEcon = 35;
  else if (taxaEcon >= 20) ptsEcon = 25;
  else if (taxaEcon >= 10) ptsEcon = 15;
  else if (taxaEcon >= 0)  ptsEcon = 5;
  fatores.push({
  ...
  if (saidasDiminuiram)   ptsEvol += 15;
  if (entradasAumentaram) ptsEvol += 10;
  fatores.push({
  ...
  let ptsReg = 0;
  if (curr.qtd_transacoes >= 10)     ptsReg = 15;
  else if (curr.qtd_transacoes >= 5) ptsReg = 10;
  else if (curr.qtd_transacoes >= 1) ptsReg = 5;
  fatores.push({
...
function gerarInsights(curr: RawPeriodData, prev: RawPeriodData): Insight[] {
