...
router.get("/:userId/monthly", async (req, res) => {
  try {
    ...
    const report = await buildReport(
      userId, inicioPeriodo, fimPeriodo, inicioAnterior, fimAnterior, "mensal"
    );
...
router.get("/:userId/weekly", async (req, res) => {
  try {
    ...
    if (req.query.inicio) {
      ...
      inicioPeriodo = new Date(Date.UTC(
        hoje.getUTCFullYear(),
        hoje.getUTCMonth(),
        hoje.getUTCDate() - diasAteSegunda
      ));
    }
    ...
    const report = await buildReport(
      userId, inicioPeriodo, fimPeriodo, inicioAnterior, fimAnterior, "semanal"
    );
